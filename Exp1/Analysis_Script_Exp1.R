...
Descriptive_general <- Descriptive_participants %>%
  dplyr::summarise(
    ...
    across(c(Age, Accuracy, RT,Probe_Response,Freq_Expected), 
...
## First saccade probability analysis -----------------------------------------
# ---- 1. Effect of time on task ----
target_variable <- "Expected"
graph_label <- tolower(target_variable)

df_logistic <- df_clean %>% 
  filter(First_Saccade_Quadrant !="") %>%  # Exclude trials with no recorded first saccade
...
fit <- glmer(First_Saccade_Quadrant_type ~ Trial_scaled + (1|Participant), 
