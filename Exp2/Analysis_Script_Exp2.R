...
Descriptive_general <- Descriptive_participants %>%
  dplyr::summarise(
    ...
    list(average = mean, sd = sd), .names = "{.col}_{.fn}")
  )
...
df_clean <- df %>% 
  ...
  mutate(
    ...
    BCEA = as.numeric(BCEA),                                                                                      # Bivariate Contour Ellipse Area: measure of gaze dispersion
    Previous_Quadrant_Expected = as.factor(Previous_Quadrant_Expected)
  ) %>%
  filter(RT > 200 & RT <= (quantile(RT, 0.75, na.rm = T) + 3 * IQR(RT, na.rm = T))) %>%  # Exclude anticipations (< 200 ms) and upper RT outliers (Q3 + 3×IQR)
  ungroup() %>% 
  mutate(Participant_Number = dense_rank(Participant)) %>%  # Assign sequential numeric IDs
...
df_nlme <- df_clean %>% 
  ...
  dplyr::select("RT", "Trial", "Bin_10","Bigram_Type", "Participant")  
...
fit <- nlme(
  ...
  data = grouped_data,
  fixed = list(yf ~ Bigram_Type, y0 ~ Bigram_Type, log_alpha ~ Bigram_Type),  # Bigram_Type modifies all three curve parameters
...
df_lme <- df_clean %>% 
  ...
  filter(Learning_Index != "",
         Trial_Probe != ""                                             # Retain only trials within the pre-probe window
  ) %>%
  dplyr::select(
...
### Variables selection
variables <- c("SD_X", "SD_Y", "BCEA")  # Gaze dispersion variables; log-transformed prior to modelling (see below)
    ...
    filter(
      ...
      case_when(
        var_name %in% c("BCEA", "SD_X", "SD_Y") ~
          is_not_outlier_IQR(.data[[var_name]]), # 1.67
        TRUE ~ is_not_outlier_IQR(.data[[var_name]]) # 1.5
      )
    ) %>%
    ungroup() %>%
